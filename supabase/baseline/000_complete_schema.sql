...
--   4. 生成时间: 2026-08-03T06:10:55.937Z
--   5. 覆盖范围: archive/001_init_tables.sql -> active/168_close_auth_review_findings.sql
-- ============================================
...
-- >>> BEGIN MIGRATION: active/150_create_version_content_snapshots.sql
...
-- <<< END MIGRATION: active/150_create_version_content_snapshots.sql

-- >>> BEGIN MIGRATION: active/151_seed_multi_version_calendar.sql
-- 151: 多版本日历元数据与历史版本快照
--
...
-- <<< END MIGRATION: active/151_seed_multi_version_calendar.sql

-- >>> BEGIN MIGRATION: active/152_add_history_review_and_import_staging.sql
...
NOTIFY pgrst, 'reload schema';
-- <<< END MIGRATION: active/152_add_history_review_and_import_staging.sql

-- >>> BEGIN MIGRATION: active/153_commit_official_import_records_atomically.sql
...
NOTIFY pgrst, 'reload schema';
-- <<< END MIGRATION: active/153_commit_official_import_records_atomically.sql

-- >>> BEGIN MIGRATION: active/154_bump_site_version_452.sql
-- 154: bump public site version and invalidate public bootstrap caches for v4.5.2.
...
SET
  value = jsonb_build_object(
    'version', ((extract(epoch from now()) * 1000)::bigint)::text,
    'scope', 'site-config',
    'reason', 'migration:154_bump_site_version_452',
    'updatedAt', now()
  )::text,
  updated_at = NOW()
WHERE key = 'public_cache_epoch';
-- <<< END MIGRATION: active/154_bump_site_version_452.sql

-- >>> BEGIN MIGRATION: active/155_guard_ambiguous_history_batch_delete.sql
...
-- >>> BEGIN MIGRATION: active/157_repair_official_non_pull_artifact.sql
...
NOTIFY pgrst, 'reload schema';
-- <<< END MIGRATION: active/157_repair_official_non_pull_artifact.sql

-- >>> BEGIN MIGRATION: active/158_bump_site_version_454.sql
-- 158: bump public site version and invalidate public bootstrap caches for v4.5.4.
...
SET
  value = jsonb_build_object(
    'version', ((extract(epoch from now()) * 1000)::bigint)::text,
    'scope', 'site-config',
    'reason', 'migration:158_bump_site_version_454',
    'updatedAt', now()
  )::text,
  updated_at = NOW()
WHERE key = 'public_cache_epoch';
-- <<< END MIGRATION: active/158_bump_site_version_454.sql

-- >>> BEGIN MIGRATION: active/166_harden_admin_profile_and_oauth_transactions.sql
...
NOTIFY pgrst, 'reload schema';
-- <<< END MIGRATION: active/166_harden_admin_profile_and_oauth_transactions.sql

-- >>> BEGIN MIGRATION: active/167_harden_account_credentials_and_identity_keys.sql
...
NOTIFY pgrst, 'reload schema';
-- <<< END MIGRATION: active/167_harden_account_credentials_and_identity_keys.sql

-- >>> BEGIN MIGRATION: active/168_close_auth_review_findings.sql
...
END;
$$;
-- <<< END MIGRATION: active/168_close_auth_review_findings.sql
