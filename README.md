...
- 版本：`v4.5.3`
- 官方导入：浏览器提交 `import-full` 后只轮询 `import-status`；服务端内部暂存并自动原子写入，不再调用同步 `import-confirm`。可精确定位的未知角色 / 武器记录会保留并在导入完成后提示核对，缺少安全归属字段的记录会跳过。
- 历史维护：支持按账号、区服、卡池和序号精确编辑 / 删除，并提供异常记录提醒与后台复核。
- 仪表盘：详细日志按需挂载，时间线与卡池阵容复用缓存；本地账号历史采用必要列并行分页读取。
