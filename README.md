...
`v4.6.0` 包含已验收的桌面首页、导航和个人 / 全服统计拆分 Demo。开发服务器启动后打开 `/?home-demo=unified`，可预览以 1366×768 为基准的首页、统一消息弹窗和可收起个人菜单；生产构建不能通过该参数启用。布局、路由、独立版本倒计时主题接口及验证范围见 [桌面 Demo 文档](docs/DESKTOP_HOME_DEMO.md)，版本交付范围见 [发布说明](docs/RELEASE_4.6.0.md)。后续首次使用教程与首页指南优化已登记为 [ONBOARDING-GUIDE-001](docs/ONBOARDING_GUIDE_PLAN.md)，待开始。
