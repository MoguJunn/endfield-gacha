...
const migrationPath = path.resolve(process.cwd(), 'supabase/migrations/177_add_extra_pool_subtypes.sql');
const splitSubtypeMigrationPath = path.resolve(
  process.cwd(),
  'supabase/migrations/179_split_reconstruction_claim_subtype.sql'
);
...
describe('extra pool subtype migration', () => {
  ...
  it('is included in the generated baseline', () => {
    ...
    expect(baseline).toContain('active/177_add_extra_pool_subtypes.sql');
    expect(baseline).toContain('active/178_seed_reconstruction_pools_and_promotion.sql');
    expect(baseline).toContain('active/179_split_reconstruction_claim_subtype.sql');
  });
