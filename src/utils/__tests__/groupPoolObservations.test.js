...
describe('six-star scope aggregation', () => {
  it('keeps first samples separate for repeated official periods under one pool ID', () => {
    const stats = build([row(1, 'a', 'p1', { pool_version: 1 }), row(2, 'low', 'p1', { pool_version: 2 }),
      row(3, 'a', 'p1', { pool_version: 2 }), row(4, 'a', 'p1', { pool_version: 2 })]);
    expect(stats.participatingAccounts).toBe(1);
    expect(category(stats, 'featured')).toMatchObject({ count: 3, nonObtainingAccounts: 0,
      first: { sampleCount: 2, mean: 1.5 }, repeat: { sampleCount: 1, mean: 1 } });
  });
