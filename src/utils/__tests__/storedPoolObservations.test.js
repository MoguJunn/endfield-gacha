...
describe('stored observation adapter', () => {
  it('separates official rerun periods without multiplying account coverage', () => {
    const history = [row(1, { pool_version: 1 }), row(2, { pool_version: 1 }),
      row(3, { pool_version: 2, character_id: 'b', rarity: 4 }), row(4, { pool_version: 2 }),
      row(5, { pool_version: null })];
    const result = buildStoredPoolObservations({ history, poolId: 'p' });
    expect(result).toMatchObject({ total: 5, participatingAccounts: 1, incompleteAccounts: 1 });
    expect(result.items.find((item) => item.itemId === 'a')).toMatchObject({
      count: 4, nonObtainingAccounts: 0, first: { sampleCount: 3, mean: 4 / 3 }, repeat: { sampleCount: 1, mean: 1 },
    });
  });
