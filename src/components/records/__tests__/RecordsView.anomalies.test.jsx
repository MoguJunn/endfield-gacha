...
vi.mock('../../../i18n/index.js', () => ({
  useI18n: () => ({ t: (key) => key, formatNumber: String, locale: 'zh-CN' }),
}));
...
describe('RecordsView history anomalies', () => {
  ...
  it('只在当前账号、区服和卡池显示提醒，并把精确记录交给编辑删除', async () => {
    ...
    expect(harness.loadHistoryAnomalies).toHaveBeenCalledWith({
      poolId: 'pool-1', gameUid: 'game-1', serverScope: 'cn-1',
    });

    fireEvent.click(screen.getByRole('button', { name: /编辑记录/ }));
    fireEvent.click(screen.getByRole('button', { name: /不是我的，删除/ }));
    ...
    await waitFor(() => {
      expect(harness.updateHistoryAnomaly).toHaveBeenCalledWith({
        anomalyId: 'anomaly-1', action: 'postpone',
      });
  ...
  it('首次读取失败时显示错误而不是静默隐藏', async () => {
    ...
    expect(await screen.findByText('待核对记录读取失败：数据库暂时不可用')).toBeInTheDocument();
  });
  ...
  it('快速切换卡池时忽略旧卡池迟到的响应', async () => {
    ...
    await act(async () => {
      resolveSecond([makeAnomaly({
        id: 'anomaly-b', record_id: 'record-b', pool_id: 'pool-b', details: { itemName: '新池记录' },
      })]);
    });
    expect(await screen.findByText('新池记录')).toBeInTheDocument();

    await act(async () => {
      resolveFirst([makeAnomaly({
        id: 'anomaly-a', record_id: 'record-a', pool_id: 'pool-a', details: { itemName: '旧池记录' },
      })]);
    });
