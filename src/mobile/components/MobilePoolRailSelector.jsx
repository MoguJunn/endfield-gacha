...
import { isPoolGroupId, parsePoolGroupId } from '../../stores/usePoolStore';
...
function toneClass(tone) {
  switch (tone) {
    ...
    default:
      return 'border-zinc-200 bg-zinc-100 text-slate-500 dark:border-white/8 dark:bg-white/[0.03] dark:text-zinc-400';
  }
}
...
function getPoolTypeConfig(pool) {
  ...
  if (type === 'weapon')
    return {
      icon: Swords,
      accent:
        'border-slate-300 bg-slate-100 text-slate-700 dark:border-slate-400/25 dark:bg-slate-400/8 dark:text-slate-200',
    };
  if (type === 'extra')
    return {
      icon: Star,
      accent: 'border-cyan-300 bg-cyan-50 text-cyan-700 dark:border-cyan-400/25 dark:bg-cyan-400/8 dark:text-cyan-300',
    };
  if (type === 'limited')
    return {
      icon: Star,
      accent:
        'border-orange-300 bg-orange-50 text-orange-700 dark:border-orange-400/25 dark:bg-orange-400/8 dark:text-orange-300',
    };
  return {
    icon: Layers,
    accent:
      'border-amber-300 bg-amber-50 text-amber-700 dark:border-amber-400/25 dark:bg-amber-400/8 dark:text-amber-300',
  };
}
...
export default function MobilePoolRailSelector() {
  ...
  const selectedPool = useMemo(() => {
    if (isPoolGroupId(currentPoolId)) {
      const groupScope = parsePoolGroupId(currentPoolId);
      return {
        id: currentPoolId,
        name:
          groupScope?.type === 'all'
            ? t('pool.selector.allOverview')
            : t('pool.card.allGroupTitle', {
                label: getPoolTypeLabel(groupScope?.type, locale, groupScope?.subtype),
              }),
        isGroupMode: true,
        type: groupScope?.type,
        subtype: groupScope?.subtype,
      };
    }
  ...
  const currentPoolLatestRecordAt = useMemo(() => {
    ...
    const timestamp = getLatestHistoryTimestampMs(
      filteredHistory.filter((item) => (item.poolId || item.pool_id) === currentPoolId)
    );
    return timestamp ? new Date(timestamp).toISOString() : null;
  ...
  const selectedPullCount = useMemo(() => {
    ...
    if (isPoolGroupId(currentPoolId)) {
      const groupScope = parsePoolGroupId(currentPoolId);
      const targetGroup = groupedPools.find((group) => group.type === groupScope?.type);
      if (groupScope?.subtype) {
        return targetGroup?.subgroups?.find((subgroup) => subgroup.subtype === groupScope.subtype)?.totalPulls || 0;
      }
  ...
  const selectedConfig =
    selectedPool && !selectedPool.isGroupMode
      ? getPoolTypeConfig(selectedPool)
      : {
          icon: Layers,
          accent:
            'border-zinc-200 bg-zinc-100 text-slate-600 dark:border-white/8 dark:bg-white/[0.04] dark:text-zinc-300',
        };
  ...
  const renderGroupOverviewOption = ({ id, groupType, label, poolCount, pulls, icon }) => {
    const active = currentPoolId === id;
    const GroupIcon = icon;
    return (
      <button
        key={id}
        type="button"
        onClick={() => handleSelectItem({ id, kind: 'group', groupType })}
        className={cx(
        ...
        <div className="min-w-0 flex-1">
          <div
            className={cx(
              ...
              active ? 'text-slate-900 dark:text-white' : 'text-slate-800 dark:text-zinc-200'
            )}
          >
            {t('pool.card.allGroupTitle', { label })}
          </div>
          <div className="mt-0.5 flex items-center gap-2 text-[10px] text-slate-500 dark:text-zinc-500">
            <span className="truncate">
              {t('pool.card.groupStats', { pools: formatNumber(poolCount), pulls: formatNumber(pulls) })}
            </span>
            <span className="shrink-0 font-mono">
              {formatNumber(pulls)} {t('dashboard.unit.pull')}
            </span>
          </div>
        </div>
    ...
    return (
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center gap-2 text-[10px] text-slate-500 dark:text-zinc-500">
            <span className="truncate">
              {shouldShowPoolFeaturedSummary(pool)
                ? pool.displayUpCharacter ||
                  getPoolSelectorFeaturedCharacters(pool, { locale }).join(' / ') ||
                  localizeEntityName(pool.up_character || pool.upCharacter || '', { locale }) ||
                  t('pool.card.overviewDesc')
                : t('pool.card.overviewDesc')}
            </span>
            <span className="shrink-0 font-mono">
              {formatNumber(pool.pullCount || 0)} {t('dashboard.unit.pull')}
            </span>
          </div>
        </div>
        {active ? <Check size={14} className="shrink-0 text-endfield-yellow" /> : null}
    ...
    <div className="space-y-1.5 overflow-visible">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="text-[10px] font-mono text-slate-500 dark:text-zinc-500">
          <span className="text-slate-900 dark:text-white">{formatNumber(visiblePoolCount)}</span>
          {visiblePoolCount !== pools.length ? <span>/{formatNumber(pools.length)}</span> : null}
        ...
        <div className="grid gap-2 text-[10px]">
          <div className="mobile-ux-card-inset flex min-w-0 items-center justify-between gap-3 px-3 py-2.5 text-slate-700 dark:text-zinc-300">
            <div className="min-w-0">
              <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500 dark:text-zinc-500">
            ...
            <span
              className={cx(
                ...
                toneClass(getFreshnessTone(currentPoolLatestRecordAt))
              )}
            >
      ...
      {showFilters && (zeroPullPoolCount > 0 || pools.length > 5) ? (
        <MobileGlassPanel
          compact
          className="space-y-2 border-zinc-200 bg-white/90 dark:border-zinc-800 dark:bg-zinc-950/70"
        >
          <div className="flex items-center gap-2">
            {zeroPullPoolCount > 0 ? (
              <button
                ...
                className={cx(
                  'rounded-full border px-3 py-2 text-[10px] font-bold uppercase tracking-[0.14em]',
                  hideZeroPullPools
                    ? 'border-emerald-400/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'
                    : 'border-zinc-200 bg-zinc-100 text-slate-500 dark:border-white/8 dark:bg-white/6 dark:text-zinc-400'
                )}
              >
            ...
            {pools.length > 5 ? (
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="mobile-ux-input py-2 pl-9 pr-9 text-xs font-mono"
                />
                {searchQuery ? (
                  <button
                    type="button"
      ...
      {pools.length === 0 ? (
        <div className="rounded-[1rem] border border-dashed border-zinc-200 bg-white/80 py-4 text-center text-sm font-mono text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-zinc-500">
        ...
        <div className="relative mt-0.5" ref={poolMenuRef}>
          <button
            ...
            className={cx(
              ...
              showPoolMenu ? 'border-endfield-yellow/40 bg-endfield-yellow/10' : 'hover:border-zinc-300'
            )}
          >
            <div className="flex min-w-0 items-center gap-3">
              <div
                className={cx(
                  ...
                  selectedPool?.isGroupMode
                    ? 'border-zinc-200 bg-zinc-100 text-slate-600 dark:border-white/8 dark:bg-white/6 dark:text-zinc-300'
              ...
              <div className="min-w-0">
                ...
                <div className="mt-0.5 flex items-center gap-2 text-[10px] text-slate-500 dark:text-zinc-500">
                  <span className="truncate">
                    {shouldShowPoolFeaturedSummary(selectedPool)
                      ? selectedPool?.displayUpCharacter ||
                        getPoolSelectorFeaturedCharacters(selectedPool, { locale }).join(' / ') ||
                        localizeEntityName(selectedPool?.up_character || selectedPool?.upCharacter || '', { locale }) ||
                        (selectedPool?.isGroupMode
                          ? t('pool.card.groupStats', {
                              pools: formatNumber(visiblePoolCount),
            ...
            <ChevronDown
              size={16}
              className={cx(
                'shrink-0 text-slate-500 transition-transform dark:text-zinc-400',
                showPoolMenu && 'rotate-180'
              )}
            />
          </button>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 flex items-center gap-2 text-[10px] text-slate-500 dark:text-zinc-500">
                        ...
                        <span className="shrink-0 font-mono">
                          {formatNumber(totalPulls)} {t('dashboard.unit.pull')}
                        </span>
                      </div>
                    </div>
                    {currentPoolId === allOverviewId ? (
                      <Check size={14} className="shrink-0 text-endfield-yellow" />
                    ) : null}
                  </button>
                ) : null}

                {groupedPools.map((group) => {
                  const groupId = getPoolGroupId(group.type);
                  const groupExpanded = resolvePoolGroupExpanded(group, currentPoolId, expandedGroups[group.type]);
                  const hasSubgroups = Array.isArray(group.subgroups);
                  const versionFoldEnabled =
                    !hasSubgroups && group.versionFold?.enabled === true && !group.disableCollapse;
                  ...
                  const displayedPools = oldPoolsExpanded ? [...directPools, ...foldedPools] : directPools;
                    ...
                    <div key={group.type} className="mobile-ux-card-inset overflow-hidden">
                      ...
                      >
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-0.5 text-[10px] font-mono text-slate-500 dark:text-zinc-500">
                              {t('pool.card.groupStats', {
                                pools: formatNumber(group.pools.length),
                                pulls: formatNumber(group.totalPulls),
                              })}
                            </div>
                          </div>
                        </div>
                        {!group.disableCollapse ? (
                          <ChevronDown
                            size={14}
                            className={cx(
                              'shrink-0 text-slate-500 transition-transform dark:text-zinc-500',
                      ...
                      {groupExpanded ? (
                        <div className="space-y-1 border-t border-zinc-200/80 p-2 dark:border-zinc-800">
                          {showOverviewOptions && (group.pools.length > 1 || group.type === 'extra')
                            ? renderGroupOverviewOption({
                                id: groupId,
                                groupType: group.type,
                                label: group.label,
                                poolCount: group.pools.length,
                                pulls: group.totalPulls,
                                icon: HeaderIcon,
                              })
                            : null}
                          ...
                          {hasSubgroups ? (
                            group.subgroups.map((subgroup) => (
                              <div
                                ...
                                className="overflow-hidden rounded-lg border border-zinc-200/80 dark:border-zinc-800"
                              >
                                <button
                                  type="button"
                                  onClick={() =>
                                    subgroup.disableCollapse
                                      ? undefined
                                      : toggleSubgroup(subgroup.groupId, subgroup.isExpanded)
                                  }
                                  className="flex w-full items-center justify-between gap-3 bg-cyan-50/60 px-3 py-2.5 text-left dark:bg-cyan-950/15"
                                >
                                  <div className="flex min-w-0 items-center gap-2">
                                    <Star size={12} className="shrink-0 text-cyan-700 dark:text-cyan-300" />
                                    <div className="min-w-0">
                                      <div className="truncate text-[11px] font-bold uppercase tracking-[0.12em] text-cyan-800 dark:text-cyan-200">
                                        {subgroup.label}
                                      </div>
                                      <div className="mt-0.5 text-[10px] font-mono text-slate-500 dark:text-zinc-500">
                                        {t('pool.card.groupStats', {
                                          pools: formatNumber(subgroup.poolCount),
                                          pulls: formatNumber(subgroup.totalPulls),
                                        })}
                                      </div>
                                    </div>
                                  </div>
                                  {!subgroup.disableCollapse ? (
                                    <ChevronDown
                                      size={14}
                                      className={cx(
                                        'shrink-0 text-cyan-700 transition-transform dark:text-cyan-300',
                                        subgroup.isExpanded && 'rotate-180'
                                      )}
                                    />
                                  ) : null}
                                </button>
                                ...
                                {subgroup.isExpanded ? (
                                  <div className="space-y-1 border-t border-zinc-200/80 p-2 dark:border-zinc-800">
                                    {showOverviewOptions
                                      ? renderGroupOverviewOption({
                                          id: subgroup.groupId,
                                          groupType: `extra:${subgroup.subtype}`,
                                          label: subgroup.label,
                                          poolCount: subgroup.poolCount,
                                          pulls: subgroup.totalPulls,
                                          icon: Star,
                                        })
                                      : null}
                                    {subgroup.pools.map(renderPoolOption)}
                                  </div>
                                ) : null}
                              </div>
                            ))
                            ...
                            <>
                              {displayedPools.map(renderPoolOption)}
                              {foldedPools.length > 0 ? (
                                <button
                                  type="button"
                                  onClick={() =>
                                    setExpandedOldPoolGroups((current) => ({
                                      ...current,
