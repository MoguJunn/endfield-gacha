...
import { formatFreshnessAbsolute, formatFreshnessRelative, getFreshnessTone } from '../../utils/dataFreshness.js';
...
import { isGameAccountSelectionMatch, localizeGameAccountServerTag } from '../../utils/gameAccountMetadata.js';
...
export default function MobileOverviewView() {
  ...
  const currentStats =
    hasAnalysisSnapshot && isAnalysisScopeCurrent && scopeOverviewStats
      ? {
          ...scopeOverviewStats,
  ...
  const currentAccount = useMemo(
    () => accounts.find((account) => isGameAccountSelectionMatch(account, effectiveGameUid)) || accounts[0] || null,
    ...
    () =>
      new Map(
        (Array.isArray(pools) ? pools : []).map((pool) => {
          ...
          return [pool.id, capabilities.isResolved ? capabilities.basePoolType : normalizePoolType(pool.type)];
        })
      ),
    [pools]
  );
  const recentSixStars = useMemo(() => {
    if (hasAnalysisSnapshot && isAnalysisScopeCurrent && Array.isArray(analysisScope?.recentSixStars)) {
  ...
  const totalSixStars = Number(currentStats?.counts?.['6'] || 0) + Number(currentStats?.counts?.['6_std'] || 0);
    ...
    <div className="h-full flex-1 overflow-y-auto overflow-x-hidden px-4 pb-24 slide-up-enter">
      ...
      <section className="mb-4 grid grid-cols-2 gap-3" aria-label={t('nav.overview')}>
        <div className="mobile-ux-card relative overflow-hidden p-4">
          <Layers
            className="absolute -bottom-4 -right-4 h-20 w-20 text-slate-200/50 dark:text-zinc-800/50"
            aria-hidden="true"
          />
          <div className="relative">
            <div className="text-[10px] font-bold tracking-widest text-slate-500 dark:text-zinc-400">
              {t('summary.metric.totalPulls')}
            </div>
            <div className="mt-1 font-mono text-3xl font-black text-slate-900 dark:text-white">
        ...
        <div className="mobile-ux-soft-card relative overflow-hidden border-amber-500/20 bg-amber-500/5 p-4 dark:border-ef-yellow/20 dark:bg-ef-yellow/5">
          ...
          <div className="relative">
            <div className="text-[10px] font-bold tracking-widest text-amber-600/80 dark:text-ef-yellow/80">
              {t('summary.metric.totalSixStars')}
            </div>
      ...
      <section className="-mx-4 mb-4">
        <div className="mb-3 flex items-center justify-between px-4">
          <h2 className="text-sm font-bold tracking-widest text-slate-900 dark:text-white">
            {t('summary.recentSixStar.title')}
          </h2>
          <button
            type="button"
            onClick={() => navigate(getMobilePathForTab('details'))}
        ...
        <div ref={recentSixStarsRef} className="flex snap-x gap-3 overflow-x-auto px-4 pb-4 scrollbar-hide">
          {recentSixStars.length > 0 ? (
            recentSixStars.map((item) => (
              <article
                key={item.id}
                className="mobile-ux-card relative w-[140px] shrink-0 snap-start overflow-hidden border-l-4 border-l-orange-500 bg-orange-50/85 p-3 dark:bg-zinc-900/70"
              >
                <div
                  className="absolute -bottom-4 -right-4 text-6xl font-black italic text-orange-500/10"
                  aria-hidden="true"
                >
                  {item.pulls}
                </div>
                <div className="relative mb-2 flex items-start justify-between gap-1">
                  <span
                    className={`px-1.5 py-0.5 text-[9px] font-bold ${item.isUp ? 'bg-orange-500 text-white' : 'bg-zinc-200 text-slate-600 dark:bg-zinc-800 dark:text-zinc-400'}`}
                  >
                    {item.isUp ? t('dashboard.timeline.badge.up') : t('dashboard.timeline.badge.off')}
                  </span>
                  <span className="max-w-[70px] truncate bg-white/50 px-1 font-mono text-[9px] text-slate-500 dark:bg-black/50 dark:text-zinc-500">
                    {item.pool}
                  </span>
                </div>
                <div className="relative mb-1 truncate text-sm font-bold text-slate-900 dark:text-white">
                  {item.name}
                </div>
                <div className="relative flex items-baseline gap-1">
                  <span className="font-mono text-2xl font-black text-orange-600 dark:text-orange-400">
                    {item.pulls}
                  </span>
                ...
                <div className="relative mt-1 text-[9px] text-slate-400 dark:text-zinc-600">
                  {formatDateTime(
                    item.date,
                    { includeYear: false, month: 'numeric', day: 'numeric' },
                    t('common.unknown')
                  )}
                </div>
              </article>
            ...
            <div className="w-full p-4 text-center text-xs italic text-slate-400 dark:text-zinc-500">
              {t('pool.selector.noRecords')}
            </div>
          )}
        </div>
      </section>
