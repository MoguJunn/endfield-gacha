...
| 导入恢复 | 官方导入已支持 Token / JSON 解析、剪贴板读取、同一 Token 重试、CN / INTL 互换、文件导入 30 分钟 `sessionStorage` 草稿恢复，以及可复制脱敏诊断；后端增量模式已有保守提前停止守卫。`v4.5.3 / 1.6.2` 已完成 CN / INTL 真实全量导入验证：浏览器提交 `import-full` 后只轮询 `import-status`，后端内部暂存并自动原子写入，可定位异常在写入后提示处理。 | 全量写入链已完成，不再列为待验证。后续只保留增量提前停止、失去安全条件时降级完整抓取、错服子区服回退、更多池级差异明细，以及别名迁移后 raw pool id 与 canonical pool id 不一致时的增量观测。 | `IMPORT-UX-002` |
