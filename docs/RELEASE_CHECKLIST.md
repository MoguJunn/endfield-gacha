...
- [ ] 若决定正式接入，先确定默认入口与 DEV 预览代码归属，审阅本任务差异并保留其他并行修改。
- [ ] 对最终拟发布改动运行相关单测、lint、build 和浏览器验证；同时检查未带查询参数的原入口、移动端与生产构建行为，不用早期阶段结果替代。
- [ ] 正式发布时更新版本 / changelog、产品截图和实际提交 / CI / 部署证据；当前预览截图不替换生产首页截图。
