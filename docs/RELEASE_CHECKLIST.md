...
本次准备版本为 `v4.6.2`，范围与已有验证见 [RELEASE_4.6.2.md](RELEASE_4.6.2.md)。以下历史发布记录及复用模板保留各自阶段含义，不代表本轮全部重新执行。

## v4.6.2 发布前状态（2026-09-24）

- [x] 本地实现包含五类合池、逐账号逐期逐对象首次后类别汇总、去重覆盖、旧指标／资源迁移、十图与头像选择、深色及动画、全桌面页面统一首页宽度。
- [x] 优化后最终全量 269 文件／1,498 项单测通过；完整 ESLint、主应用与抽奖子应用构建、PGlite 统计 SQL 行为与覆盖 186 个迁移的 baseline 验证已有通过证据，优化改动另通过 ESLint 与主应用构建。
- [x] 生产依赖 `npm audit --omit=dev --audit-level=high` 为 0 漏洞；内部镜像不支持 audit，改用官方 registry 成功执行。
- [x] 工作区 `_docs/公告.md` 已准备 v4.6.2 中英文正文并保留 v4.6.0；尚未在网站发布，该文件不进入仓库提交。
- [x] SSH 只读核对：生产数据库无 `statistics_jobs` / `statistics_snapshots`，无统计 Worker；现站仍为 `v4.6.0`。
- [x] 已有真实浏览器验证覆盖桌面 1366～3840、合池 360～1920 宽度；指南仅为 DEV 预览，未接入真实动作。
- [ ] 维护者审查：Git 工作流程要求至少一名审查者，仓库目前仅 `MoguJunn` 一个管理员账号，作者无法批准自己的 PR；需要其他维护者审查或记录流程例外。
- [ ] 从 `feat/banner-stats-and-guide` 创建发布 PR 并通过 CI；按用户安排暂停合并，等待并行的新卡池适配完成和后续合并指令，不开启自动合并、不修改自动部署配置。
- [ ] 重新确认生产主机容量（主库主机约 6GB／可用 1665MiB、swap 3272/4095；备选主机 2GB／可用 418MiB；第三台超时）。优化后连续五组合池本地进程峰值 810MiB，须结合实时数据库负载确认达标主机，再执行统计迁移与启用常驻 Worker。
- [ ] 公开单池、五类合池、旧统计和个人范围均已完成 `public-statistics-v4` 预热，随后才启用前端／API。
- [ ] 发布网站公告，并核验生产版本、公告、队列延迟、最大任务耗时及真实公共／个人页面，补充部署证据。

本轮尚未发布、合入 main 或部署；已有本地测试不关闭上述发布与生产门禁。
...
## v4.6.0 发布结果（2026-09-07 核对）

- [x] 主站包、锁文件、沙盒版本、构建日期、当前文档和 changelog 已同步。
- [x] 隔离候选 lint、251 个文件 / 1363 项单测、包含抽奖子应用的完整构建通过。
- [x] PR #32 已去除桌面新版的 DEV 限制，默认新版并支持持久化双向切换；Neptune-520 共同贡献署名保留。
- [x] PR #31 / #32 均在 CI 通过后合入 main；本地 main / origin/main 为 `fca9314b`，`v4.6.0` 标签保留在初始发布提交 `9babb6c3`。
- [x] GitHub 自动 Production Ready，正式域名指向最新部署；生产版本与构建日期已按单独授权同步。
- [x] 主页切换修复通过 8 项定向测试、完整 lint / build、PR / main CI，以及生产构建和正式站点浏览器验证。浏览器预设启动校验有效状态，不代表真实验证码回归。
- [x] `fix/v4.6-home-default` 确认已在 main 祖先链后闭拢，本地与远端引用已删除。
...
下列记录保存 `DESKTOP-HOME-001` 首次本地提交时的验证情况；当时的预览限制已由 PR #32 解除，当前发布结果见上面的 v4.6.0 节。具体合同见 [DESKTOP_HOME_DEMO.md](DESKTOP_HOME_DEMO.md)。

- [x] 本地桌面布局与交互获用户确认；1366×768 基线、较大视口、窄窗分区、中英文 / 亮暗、侧栏收起、消息动作与减少动态效果已定向验证。
- [x] 个人 / 全服统计及图鉴锁定来源，个人数据边界与公共统计隔离；统计拆分阶段 11 项相关测试通过。
- [x] 2026-09-05 提交前 `npm run lint` 与主站 `vite build` 通过；此前桌面浏览器验证通过。
- [ ] 同次全量单测 1383 项通过、1 项失败：`siteOverview.test.js` 的旧英文名断言与未纳入本轮提交的版本英文名修改不一致。需要在该并行修改收口时同步核验，不能将本次工作树全量结果写成全部通过。
- [x] 已同步本地预览专题、README、贡献指南、架构、代码地图、收口记录及工作区 todo / handoff / 任务档案。
- [x] 新增 `ONBOARDING-GUIDE-001` 首次使用教程与首页指南优化待办；本次只登记，尚未实施。
- [x] 正式接入已完成：默认新版、经典版可切换，生产可用的懒加载与并行修改隔离已核对。
- [x] 最终发布改动已运行单测、lint、build 和浏览器验证；入口无需查询参数，移动端行为保留。
- [x] 版本 / changelog 与实际提交、CI、部署证据已更新；早期截图保持历史标识，本次未重新制作产品截图。
