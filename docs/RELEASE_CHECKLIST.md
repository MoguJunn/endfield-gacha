...
- [x] 恢复候选含重构期次修复：275文件／1535项单测、改动 ESLint、PGlite统计 SQL 与188迁移基线验证通过；PR #37 首轮完整 CI 通过。
...
- [x] 正确主机连接已确认；生产数据库备份、两份统计迁移、独立 Worker 与定时器已安装。公开34单池、5合池及旧统计均已预热，实际读取处理器验证通过。
- [x] 已有真实浏览器验证覆盖桌面 1366～3840、合池 360～1920 宽度；指南仅为 DEV 预览，未接入真实动作。
- [x] 用户授权继续生产准备与发布；无强制他人审批保护，不伪造作者自批，保留 PR 与 CI 证据。
- [ ] PR #35 已上线，PR #34 曾因依赖未就绪由 #36 回退；恢复 PR #37 等预热与最终 CI 完成再合并。
- [x] 国际服主机采用1 CPU／1200MiB硬上限、768MiB堆的只读 Node24 容器，禁止额外 Swap；现有服务不关闭。运行时最大合池观测约684MiB，持续监控资源。
- [ ] 公开单池、五类合池、旧统计和个人范围均已完成 `public-statistics-v4` 预热，随后才启用前端／API。
- [ ] 发布网站公告，并核验生产版本、公告、队列延迟、最大任务耗时及真实公共／个人页面，补充部署证据。

本轮生产准备已执行，恢复读端仍须以预热和部署验证为准，不能仅凭 CI 通过提前上线。
