...
用户反馈大屏首页过空，并已确认调整效果。CSS 在保留下面 1366×768 基线的基础上，为新版首页新增大屏规则：宽度至少 1920px 时，顶栏、正文、底栏共用 `clamp(1366px, 78vw, 1920px)`；宽度至少 3000px 时改为 `min(74vw, 2560px)`。宽至少 1920px 且高至少 1000px 时，卡片区采用 `clamp(540px, 54dvh, 1280px)`，标题、头像、倒计时与快捷入口适度放大；活动卡放大另要求高度至少 1200px，避免低高度窗口裁切内容。其他桌面工作区继续使用原有宽度。
...
- 统一宽度适用于新版桌面壳层；“无需纵向下滑”的验收仅针对首页，长统计、设置或管理页面仍可正常滚动。
