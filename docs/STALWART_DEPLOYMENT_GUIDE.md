...
Phase A–D 已在本认证 worktree 本地完成，隔离环境中的 GitHub 登录/绑定/解绑/解绑后拒绝/重绑核心浏览器闭环也已通过，但候选尚未合入或部署。真实发信前仍需确定认证迁移最终编号，并部署邮箱归属、首次设密和临时凭据数据库能力。测试邮件只证明投递链路，不能证明账号归属或 OAuth/Session 安全闭环；LinuxDo 在公共认证发布前保持关闭。
