...
桌面 Demo 已获用户验收，并随 `v4.6.0` 纳入主线。启动开发服务器后打开 `/?home-demo=unified`；入口同时要求 Vite DEV 和查询参数，生产构建无法激活。“退出预览”回到原首页。该参数只选择界面，隔离演示数据仍由上面的贡献者沙盒配置提供。

桌面预览以 1366×768 / 100% 缩放为基准，顶栏、主内容与底栏最大宽度 1366px。上方引导区填充剩余高度，卡片区保留当前寻访、原生抽奖、原生轮换和独立版本倒计时；较小窗口用分区页签。个人卡池分析位于 `/dashboard?home-demo=unified`，个人概览增加 `&view=overview`，全服统计位于 `/summary?home-demo=unified`，两者锁定各自数据源。宽屏个人菜单在正文外侧且可收起。

完整布局、导航 / 消息语义、主题接口与分阶段验证见 [DESKTOP_HOME_DEMO.md](DESKTOP_HOME_DEMO.md)。移动端原行为保留，长内容页仍可滚动；本地 Demo 验收不表示全站治理或正式发布完成。
...
- 纳入已验收的桌面首页、统一导航和四类消息弹窗预览，支持 1366×768 基线、容器自适应、主题和减少动态效果。
- 预览中的个人概览、卡池分析与全服统计锁定各自数据源；提供可收起个人菜单和页面动效。
- 统一主站包、锁文件、本地版本默认值和沙盒版本为 `4.6.0`，同步当前文档和构建日期。
- 桌面 Demo 继续仅在 Vite DEV 且 `home-demo=unified` 时启用；首次使用教程与首页指南优化已登记，尚未实施。
