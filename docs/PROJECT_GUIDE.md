...
桌面 Demo 已获用户验收，并于 2026-09-05 保存于本地分支 `feat/v4.5-desktop-home-demo`，尚未推送或发布。启动开发服务器后打开 `/?home-demo=unified`；入口同时要求 Vite DEV 和查询参数，生产构建无法激活。“退出预览”回到原首页。该参数只选择界面，隔离演示数据仍由上面的贡献者沙盒配置提供。
